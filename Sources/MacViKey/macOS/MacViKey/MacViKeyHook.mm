...
extern "C" BOOL MacViKeyIsEventTapAlive(void);
os_log_t macViKeyLog;
int _macViKeyTapDisabledCount = 0;
...
extern "C" {
    ...
    void queryFrontMostApp();   //dinh nghia ben duoi

    NSString* MacViKeyCurrentAppName() {
        NSRunningApplication* app = [[NSWorkspace sharedWorkspace] frontmostApplication];
        return app.localizedName ?: _frontMostApp;
    }

    NSString* MacViKeyCurrentAppBundle() {
    ...
    void MacViKeyInit() {
        ...
        //load saved data
        //Bo dau tu do khoa TAT: khong doc prefs, va menu cung khong con muc nay
        //(xem macViKeyNodeEnabled: trong AppDelegate.m).
        vFreeMark = 0;
        ...
        LOAD_DATA(vTempOffEngineByHotKey, vTempOffEngineByHotKey);
        
        LOAD_DATA(vFixChromiumBrowser, vFixChromiumBrowser);
        
        LOAD_DATA(vPerformLayoutCompat, vPerformLayoutCompat);

        //Cac tuy chon da an khoi menu: khoa cung o day. Ep sau khi doc prefs de
        //gia tri cu khong con hieu luc; ghi lai prefs cho bang dieu khien thay
        //dung the.
        vFixRecommendBrowser = 1;   //luon bat: chong gach chan goi y an chu
        ...
        NSUserDefaults *lockedPrefs = [NSUserDefaults standardUserDefaults];
        [lockedPrefs setInteger:vUpperCaseFirstChar forKey:@"UpperCaseFirstChar"];
        [lockedPrefs setInteger:vAllowConsonantZFWJ forKey:@"vAllowConsonantZFWJ"];
        [lockedPrefs setInteger:vUseSmartSwitchKey forKey:@"UseSmartSwitchKey"];
        [lockedPrefs setInteger:vTempOffEngineByHotKey forKey:@"vTempOffEngineByHotKey"];
        [lockedPrefs setInteger:vOtherLanguage forKey:@"vOtherLanguage"];
        [lockedPrefs setInteger:vFreeMark forKey:@"FreeMark"];
        [lockedPrefs setInteger:vFixRecommendBrowser forKey:@"FixRecommendBrowser"];
        [lockedPrefs setInteger:vSendKeyStepByStep forKey:@"SendKeyStepByStep"];
        [lockedPrefs setInteger:vFixChromiumBrowser forKey:@"vFixChromiumBrowser"];
        [lockedPrefs setInteger:vPerformLayoutCompat forKey:@"vPerformLayoutCompat"];
    ...
    void queryFrontMostApp() {
        if ([[[NSWorkspace sharedWorkspace] frontmostApplication].bundleIdentifier compare:MACVIKEY_BUNDLE] != 0) {
    ...
    //MacViKey P0-2: CGWindowListCopyWindowInfo liet ke MOI cua so tren man hinh.
    //Do duoc: p50 0.216ms, max 3.0ms voi 15 cua so, va tang tuyen tinh theo so cua so.
    //Ham nay truoc day chay 2 lan cho MOI phim co bo dau, ngay trong duong dong bo
    //cua toan he thong. Cache lai voi TTL ngan: nguoi dung khong the mo Spotlight
    //roi go xong mot tu trong 250ms.
    static BOOL _spotlightVisibleCached = NO;
    static CFAbsoluteTime _spotlightCheckedAt = 0;
    #define SPOTLIGHT_CACHE_TTL 0.25

    BOOL isSpotlightVisibleUncached() {
    ...
    BOOL isSpotlightVisible() {
        ...
        if (now - _spotlightCheckedAt < SPOTLIGHT_CACHE_TTL)
            return _spotlightVisibleCached;
        _spotlightCheckedAt = now;
        _spotlightVisibleCached = isSpotlightVisibleUncached();
        return _spotlightVisibleCached;
    }

    //Doi app -> trang thai Spotlight chac chan da doi, bo cache ngay.
    void invalidateSpotlightCache() {
        _spotlightCheckedAt = 0;
    }
     ...
     * NSString thay vi ban CGEvent - de dua cho duong AX.
     */
    NSString* BuildResultString() {
        unichar buf[MAX_BUFF * 2];
        int n = 0;
        for (int k = pData->newCharCount - 1; k >= 0 && n < MAX_BUFF * 2 - 1; k--) {
            Uint32 ch = pData->charData[k];
            if (ch & PURE_CHARACTER_MASK)
                buf[n++] = (unichar)(ch & CHAR_MASK);
            else if (!(ch & CHAR_CODE_MASK))
                buf[n++] = (unichar)keyCodeToCharacter(ch);
            else
                buf[n++] = (unichar)(ch & CHAR_MASK);
        }
        //Khi engine yeu cau khoi phuc, phim vua bam duoc tra lai nguyen trang.
        if (pData->code == vRestore || pData->code == vRestoreAndStartNewSession) {
            Uint16 c = keyCodeToCharacter(_keycode |
                (((_flag & kCGEventFlagMaskAlphaShift) || (_flag & kCGEventFlagMaskShift)) ? CAPS_MASK : 0));
            if (c == 0) return nil;   //phim dieu khien - duong AX khong xu ly duoc
            if (n < MAX_BUFF * 2 - 1) buf[n++] = (unichar)c;
        }
        return [NSString stringWithCharacters:buf length:n];
    }

    /**
     * Thu thay the van ban qua Accessibility API.
     * Thanh cong -> khong can gia lap phim nao het (khong Backspace, khong
     * SendEmptyCharacter, khong SendShiftAndLeftArrow).
     */
    bool TryAccessibilityPath() {
        if (!MacViKeyAXIsEnabled()) return false;
        if (pData->code != vWillProcess && pData->code != vRestore &&
            pData->code != vRestoreAndStartNewSession) return false;
        if (pData->backspaceCount >= MAX_BUFF) return false;

        NSString* result = BuildResultString();
        if (result == nil) return false;

        if (MacViKeyAXReplaceText(pData->backspaceCount, result)) {
    ...
    bool checkHotKey(int hotKeyData, bool checkKeyCode=true) {
        ...
        if (HAS_SHIFT(hotKeyData) ^ GET_BOOL(_lastFlag & kCGEventFlagMaskShift))
            return false;
        //MacViKey: ho tro to hop fn + shift (Engine goc khong biet den phim fn).
        if (MACVIKEY_HAS_FN(hotKeyData) ^ GET_BOOL(_lastFlag & kCGEventFlagMaskSecondaryFn))
            return false;
        if (checkKeyCode) {
     ...
     * MAIN Callback.
     */
    CGEventRef MacViKeyCallback(CGEventTapProxy proxy, CGEventType type, CGEventRef event, void *refcon) {
        //MacViKey P0-1: macOS da tat tap cua chung ta -> bat lai ngay, neu khong bo go chet han.
        ...
        if (type == kCGEventKeyDown) {
            ...
            if (_lastFlag == 0 || _lastFlag < _flag) {
                ...
                if (vTempOffEngineByHotKey && !_hasJustUsedHotKey && _lastFlag & kCGEventFlagMaskCommand) {
                    vTempOffEngine();
                }
                _lastFlag = 0;
        ...
        if (type == kCGEventKeyDown) {
            ...
            if (pData->code == vDoNothing) { //do nothing
                ...
                BOOL _spotlight = isSpotlightVisible();
                BOOL _useRecommendWorkaround = vFixRecommendBrowser && !_spotlight &&
                        (_topApp == nil || ![_recommendWorkaroundDisabledApp containsObject:_topApp]);
                BOOL _useSelectionReplacement = _spotlight ||
                        (_topApp != nil && [_recommendWorkaroundDisabledApp containsObject:_topApp]);

                //fix autocomplete
                if (_useRecommendWorkaround && pData->extCode != 4) {
                    if (vFixChromiumBrowser && [_unicodeCompoundApp containsObject:_topApp]) {
                ...
                if (_useSelectionReplacement && pData->backspaceCount > 0) {
