...
- (IBAction)onConvertButton:(id)sender {
    if ([MacViKeyManager quickConvert]) {
        if (!convertToolDontAlertWhenCompleted) {
            [MacViKeyManager showMessage: self.view.window message:@"Chuyển mã thành công!" subMsg:@"Kết quả đã được lưu trong clipboard."];
        }
    } else {
        [MacViKeyManager showMessage: self.view.window message:@"Không có dữ liệu trong clipboard!" subMsg:@"Hãy sao chép một đoạn text để chuyển đổi!"];
    }
}
