...
static void convert(const string &str, vector<Uint32> &outData) {
  ...
  for (int i = 0; i < data.size(); i++) {
    ...
    for (map<Uint32, vector<Uint16>>::iterator it = _codeTable[0].begin();
         it != _codeTable[0].end(); ++it) {
      kSign = -1;
      k = 0;
...
static bool modifyCaseUnicode(Uint32 &code, const bool &isUpperCase = true) {
  ...
  for (map<Uint32, vector<Uint16>>::iterator it =
           _codeTable[vCodeTable].begin();
       it != _codeTable[vCodeTable].end(); ++it) {
...
void getAllMacro(vector<vector<Uint32>> &keys, vector<string> &macroTexts,
...
void saveToFile(const string &path) {
  ...
  myfile << ";Compatible Macro Data file ***\n";
  for (std::map<vector<Uint32>, MacroData>::iterator it = macroMap.begin();
       it != macroMap.end(); ++it) {
